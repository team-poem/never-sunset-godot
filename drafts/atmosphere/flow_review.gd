...
func run():
	...
	check(not game.ending_notes.is_empty(), "unread arrival survives ending selection")
	var ending_save = game._capture_save()
	var transcript = game.ending_notes.duplicate()
	game._restore_snapshot(ending_save)
	check(game.ending_notes == transcript, "ending transcript survives saved resume")
	game._on_action("__ending_notes")
	check(game.mode == "dialog" and game.ui.modal.visible and game.ui.modal_body.text == "\n\n".join(transcript), "ending transcript is an accessible reading page")
	game._on_action("__ending_return")
	check(game.mode == "ending", "transcript returns to ending")
